...
function composeChanges(cm, oldChange, change) {
    ...
    var affectedMiddle = (0, CoordinateChanges_1.preEditToPostEditChangeRange)(oldChange);
    ...
    if (rangesOverlap(affectedMiddle, change)) {
        ...
        if ((0, Util_1.rangesEqual)(affectedMiddle, change)) {
            ...
            var removed = void 0;
            var text = void 0;
            var changeToInOrigCoords = (0, CoordinateChanges_1.convertPointToBeforeChange)(change.to, oldChange);
            if ((0, Util_1.posCmp)(changeToInOrigCoords, oldChange.to) <= 0) {
                ...
                var oldChangeToInMiddleCoords = (0, CoordinateChanges_1.convertPointToAfterChange)(oldChange.to, oldChange);
        ...
        else if ((0, Util_1.posCmp)(change.from, affectedMiddle.from) < 0) {
        ...
        if (rangeIsAfterInclusive(affectedMiddle, change)) {
            ...
            var middleStartPos = (0, CoordinateChanges_1.preEditToPostEditChangeRange)(change).to;
            var middleEndPos = (0, CoordinateChanges_1.convertPointToAfterChange)(oldChange.from, change);
            ...
            var finalTo = (0, CoordinateChanges_1.convertPointToBeforeChange)(change.to, oldChange);
...
function rangeIsAfterInclusive(range1, range2) {
    return (0, Util_1.posCmp)(range2.to, range1.from) <= 0; // TODO: check this
}
// Return true if range2 is completely contained within range1
function rangeContains(range1, range2) {
    return (0, Util_1.posInsideRangeInclusive)(range2.from, range1) && (0, Util_1.posInsideRangeInclusive)(range2.to, range1);
}
function joinWithoutNewlines(list1, list2) {
    var ret = list1.slice(0, list1.length - 1);
    ret.push((0, Util_1.last)(list1) + list2[0]);
    ret.push.apply(ret, list2.slice(1));
    return ret;
}
function splitTextAt(text, line, ch) {
    var firstPart = __spreadArray(__spreadArray([], text.slice(0, line), true), [text[line].slice(0, ch)], false);
    var secondPart = __spreadArray([text[line].slice(ch)], text.slice(line + 1), true);
